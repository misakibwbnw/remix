export default function DeleteAlert(props): JSX.Element {
  // const { id } = useLoaderData<typeof loader>();

  const handleClickClose = event => {
    const value = event.target.value;
    props.handleClickClose(value);
  };
            ...
            <div className="w-72 h-fit p-4 absolute top-0 left-0 right-0 bottom-0 m-auto bg-white shadow-lg rounded-2xl dark:bg-gray-800">
              <div className="w-full h-full text-center">
                <div className="flex flex-col justify-between h-full">
                  ...
                  <p className="mt-4 text-xl font-bold text-gray-800 dark:text-gray-200">削除</p>
                  ...
                  <div className="grid grid-cols-2 items-center justify-between gap-4 mt-8">
                    <div>
                      <button
                        type="button"
                        className="py-2 px-4 w-full bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg "
                        onClick={() => handleClickClose('submit')}
                      >
                        削除
                      </button>
                    </div>
